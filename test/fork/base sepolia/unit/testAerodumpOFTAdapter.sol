...
contract AerodumpOFTAdapterTest is StdCheats, StdUtils, Test, Script {
    ...
    function testLockTokens() public {
        ...
        (uint256 amountSent, uint256 amountRecievedByRemote) =
            adapter.lockTokens(1356, 20 * 1e6, 19 * 1e6, uint32(vm.envUint("BASE_SEPOLIA_CHAIN_ID")));
        assertEq(amountSent, 20 * 1e6);
        assertEq(amountRecievedByRemote, 20 * 1e6);
        assertEq(usdc.balanceOf(address(adapter)), 20 * 1e6);
        AerodumpOFTAdapter.project memory project = adapter.getProjectDetailsForProjectOwner();
    ...
    function testqueueAirdropWithEqualDistribution() public {
        ...
        adapter.lockTokens(1357, 20 * 1e6, 19 * 1e6, uint32(vm.envUint("BASE_SEPOLIA_CHAIN_ID")));
        adapter.queueAirdropWithEqualDistribution(1357, recipients, uint32(vm.envUint("BASE_SEPOLIA_CHAIN_ID")));
        ...
        AerodumpOFTAdapter.project memory project2 = adapter.getProjectDetailsForProjectOwner();
        ...
        AerodumpOFTAdapter.project memory project3 = adapter.getProjectDetailsForProjectOwner();
        ...
        AerodumpOFTAdapter.project memory project4 = adapter.getProjectDetailsForProjectOwner();
